(function () {
  ...
  function loadImageProfiles() {
    ...
    $.ajax(addHostAndAuthType(IMAGE_PROFILES_URL))
      .done(handler)
      .fail(function () {
  ...
  function addImageProfilesRadioGroup() {
    ...
    _.each(imageProfiles, function (crops, profileName) {
      html =
        html +
        '<coral-radio name="dyn-image-profiles" value="' +
        profileName +
        '">' +
        profileName +
        '</coral-radio>';
  ...
  function addSmartCropsRadioGroup() {
    ...
    var html =
      '<coral-radio name="dyn-renditions" value="original" checked>Original</coral-radio>';
    ...
    _.each(imageProfiles, function (crops, profileName) {
      ...
      _.each(crops, function (crop) {
        html =
          html +
          '<coral-radio name="dyn-renditions" value="' +
          crop +
          '">' +
          crop +
          '</coral-radio>';
  ...
  function createDialog() {
    var selUrl = $('#aem-host-url').val() + ASSET_SELECTOR_URL,
      html =
        "<iframe width='1500px' height='800px' frameBorder='0' src='" +
        selUrl +
        "'></iframe>";
  ...
  function setDynamicRenditionImage() {
    ...
    function handler(data) {
      ...
      var metadata = data['jcr:content']['metadata'],
        dynRendUrl =
          metadata['dam:scene7Domain'] +
          'is/image/' +
          metadata['dam:scene7File'] +
          ':' +
          dynRendition;
